# Hand-maintained domain models (originally generated by datamodel-codegen from
# action-configs.json, but since edited). Field names are camelCase and have
# DIVERGED from the snake_case schema, so do NOT regenerate this file over the
# runtime package — it would rename fields and break the engine/frontend.
# Shared coercion/validation lives in floweb_models._base.
...
class BaseActionConfig(FlowebActionBaseModel):
    # Inherits populate_by_name, coerce_numbers_to_str, and the string-field
    # coercion validator from FlowebActionBaseModel (floweb_models._base).
    image: str | None = None
    """
...
class ApiCallConfig(BaseActionConfig):
    ...
    """
    Optional path into the response to store in the output variable, e.g.
    `data.token`, `headers["Content-Type"]`, `data.items[0].id`. The path is
    applied to the response object {status_code, headers, data, url} (body
    fields live under `data.`). When empty, the full response object is stored.
    """
